...
assert.match(apiClient, /DEPLOYMENT_PAGE_SIZE = 100/);
assert.match(apiClient, /MAX_DEPLOYMENT_PAGES = 100/);
assert.match(apiClient, /\?limit=\$\{DEPLOYMENT_PAGE_SIZE\}&offset=\$\{offset\}/);
assert.match(apiClient, /seenIds/);
assert.match(apiClient, /deployment_page_limit_exceeded/);
...
assert.match(managementProxy, /allowDeploymentPagination:\s*true/);
assert.match(managementProxy, /DEPLOYMENT_PAGE_LIMIT = 100/);
assert.match(managementProxy, /key !== "limit" && key !== "offset"/);
assert.match(managementProxy, /limit > DEPLOYMENT_PAGE_LIMIT/);
assert.match(managementProxy, /\?limit=\$\{limit\}&offset=\$\{offset\}/);
